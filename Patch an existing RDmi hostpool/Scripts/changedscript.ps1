    ...
    Write-Log -Message "Setting up the location of Deployagent folder"
       ...
       $obj"
            ...
            foreach($sh in $allShsNames){
                ...
                $unMangedDomain=Invoke-Command -computer $DControllerVM -credential $AdminCredentials -scriptblock {$process=Get-Service;return $process} -ErrorAction SilentlyContinue
                if($unMangedDomain)
                {
